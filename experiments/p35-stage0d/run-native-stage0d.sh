...
STAGE0D_CAPABILITY_DELTA="FUSE_EXTENSION_AND_PATH_EXTENSION"
PATH_EXTENSION_CLASS="NONE"
PATH_EXTENSION_SCOPE="NONE"
PATH_EXTENSION_SCOPE_GATE="FAIL"
...
GUEST_COMPLETION_SEEN="NOT_RUN"
GUEST_BOOT_TIMEOUT="NOT_RUN"
GUEST_INIT_STARTED="NOT_RUN"
GUEST_EARLY_USERSPACE_READY="NOT_RUN"
GUEST_VIRTIOFS_DEVICE_SEEN="NOT_RUN"
GUEST_VIRTIOFS_MOUNT_ATTEMPTED="NOT_RUN"
GUEST_VIRTIOFS_MOUNT_RESULT="NOT_RUN"
GUEST_HOST_READ_ATTEMPTED="NOT_RUN"
GUEST_HOST_READ_RESULT="NOT_RUN"
GUEST_HOST_WRITE_ATTEMPTED="NOT_RUN"
GUEST_HOST_WRITE_RESULT="NOT_RUN"
GUEST_ESCAPE_PROBES_STARTED="NOT_RUN"
GUEST_NETWORK_PROBE_STARTED="NOT_RUN"
STAGE0D_GUEST_DONE="NOT_RUN"
...
emit_report() {
  ...
  echo "PATH_EXTENSION_CLASS=$PATH_EXTENSION_CLASS"
  echo "PATH_EXTENSION_SCOPE=$PATH_EXTENSION_SCOPE"
  echo "PATH_EXTENSION_SCOPE_GATE=$PATH_EXTENSION_SCOPE_GATE"
  ...
  echo "GUEST_COMPLETION_SEEN=$GUEST_COMPLETION_SEEN"
  echo "GUEST_BOOT_TIMEOUT=$GUEST_BOOT_TIMEOUT"
  echo "GUEST_INIT_STARTED=$GUEST_INIT_STARTED"
  echo "GUEST_EARLY_USERSPACE_READY=$GUEST_EARLY_USERSPACE_READY"
  echo "GUEST_VIRTIOFS_DEVICE_SEEN=$GUEST_VIRTIOFS_DEVICE_SEEN"
  echo "GUEST_VIRTIOFS_MOUNT_ATTEMPTED=$GUEST_VIRTIOFS_MOUNT_ATTEMPTED"
  echo "GUEST_VIRTIOFS_MOUNT_RESULT=$GUEST_VIRTIOFS_MOUNT_RESULT"
  echo "GUEST_HOST_READ_ATTEMPTED=$GUEST_HOST_READ_ATTEMPTED"
  echo "GUEST_HOST_READ_RESULT=$GUEST_HOST_READ_RESULT"
  echo "GUEST_HOST_WRITE_ATTEMPTED=$GUEST_HOST_WRITE_ATTEMPTED"
  echo "GUEST_HOST_WRITE_RESULT=$GUEST_HOST_WRITE_RESULT"
  echo "GUEST_ESCAPE_PROBES_STARTED=$GUEST_ESCAPE_PROBES_STARTED"
  echo "GUEST_NETWORK_PROBE_STARTED=$GUEST_NETWORK_PROBE_STARTED"
  echo "STAGE0D_GUEST_DONE=$STAGE0D_GUEST_DONE"
...
chmod 0777 "$ALLOWED/share" "$ALLOWED/share/write-target" "$ALLOWED/write-target"
echo "STAGE0D_CAN_READ_${STAGE0D_RUN_ID}" > "$ALLOWED/can-read.txt"
STAGE0D_HOST_READ_MARKER="STAGE0D_HOST_READ_${STAGE0D_RUN_ID}"
echo "$STAGE0D_HOST_READ_MARKER" > "$ALLOWED/share/host-read.txt"
...
EXPECTED_HOST_MARKER="$STAGE0D_HOST_READ_MARKER"
...
if [ "$PROFILE_GENERATION_RC" -eq 0 ] && [ -f "$PROFILE_PATH" ]; then
  ...
  CANONICAL_SHARE="$(realpath "$ALLOWED/share")"
  PROFILE_BODY="$(grep -v '^;' "$PROFILE_PATH" | tr '\n' ' ')"

  # Verify exact class
  if [[ "$PROFILE_BODY" == *"extension-class \"com.apple.app-sandbox.read-write\""* ]]; then
    PATH_EXTENSION_CLASS="com.apple.app-sandbox.read-write"
  else
    PATH_EXTENSION_CLASS="INVALID"
  fi

  FILE_ISSUE_SUBPATH="$(grep -A 3 "file-issue-extension" "$PROFILE_PATH" | grep "subpath" | sed -E 's/.*subpath "([^"]+)".*/\1/' || echo "")"

  # Verify exact canonical share subpath
  if [ "$FILE_ISSUE_SUBPATH" = "$CANONICAL_SHARE" ]; then
    PATH_EXTENSION_SCOPE="$CANONICAL_SHARE"
  else
    PATH_EXTENSION_SCOPE="INVALID"
  fi

  # Verify no wildcard extension class or subpath, and no broader scope
  if [ "$PATH_EXTENSION_CLASS" = "com.apple.app-sandbox.read-write" ] && \
     [ "$PATH_EXTENSION_SCOPE" = "$CANONICAL_SHARE" ] && \
     [ "$FILE_ISSUE_SUBPATH" != "$ALLOWED" ] && \
     [ "$FILE_ISSUE_SUBPATH" != "$CANONICAL_RUN_DIR" ] && \
     [ "$FILE_ISSUE_SUBPATH" != "/private/tmp" ] && \
     [ "$FILE_ISSUE_SUBPATH" != "/tmp" ] && \
     [ "$FILE_ISSUE_SUBPATH" != "/Users" ] && \
     [ "$FILE_ISSUE_SUBPATH" != "/" ] && \
     [[ ! "$PROFILE_BODY" =~ (extension-class[[:space:]]+\"[^\"]*\*|\(extension-class[[:space:]]+\*) ]] && \
     [[ ! "$PROFILE_BODY" =~ (subpath[[:space:]]+\"[^\"]*\*|\(subpath[[:space:]]+\*) ]]; then
    PATH_EXTENSION_SCOPE_GATE="PASS"
  else
    PATH_EXTENSION_SCOPE_GATE="FAIL"
    BLOCK_REASON="PATH_EXTENSION_SCOPE_INVALID"
    STAGE0D_RESULT="BLOCKED"
    exit 2
  fi
else
...
GUEST_COMPLETION_SEEN="$(get_json "$OUTPUT_JSON" GUEST_COMPLETION_SEEN)"
GUEST_BOOT_TIMEOUT="$(get_json "$OUTPUT_JSON" GUEST_BOOT_TIMEOUT)"
GUEST_INIT_STARTED="$(get_json "$OUTPUT_JSON" GUEST_INIT_STARTED)"
GUEST_EARLY_USERSPACE_READY="$(get_json "$OUTPUT_JSON" GUEST_EARLY_USERSPACE_READY)"
GUEST_VIRTIOFS_DEVICE_SEEN="$(get_json "$OUTPUT_JSON" GUEST_VIRTIOFS_DEVICE_SEEN)"
GUEST_VIRTIOFS_MOUNT_ATTEMPTED="$(get_json "$OUTPUT_JSON" GUEST_VIRTIOFS_MOUNT_ATTEMPTED)"
GUEST_VIRTIOFS_MOUNT_RESULT="$(get_json "$OUTPUT_JSON" GUEST_VIRTIOFS_MOUNT_RESULT)"
GUEST_HOST_READ_ATTEMPTED="$(get_json "$OUTPUT_JSON" GUEST_HOST_READ_ATTEMPTED)"
GUEST_HOST_READ_RESULT="$(get_json "$OUTPUT_JSON" GUEST_HOST_READ_RESULT)"
GUEST_HOST_WRITE_ATTEMPTED="$(get_json "$OUTPUT_JSON" GUEST_HOST_WRITE_ATTEMPTED)"
GUEST_HOST_WRITE_RESULT="$(get_json "$OUTPUT_JSON" GUEST_HOST_WRITE_RESULT)"
GUEST_ESCAPE_PROBES_STARTED="$(get_json "$OUTPUT_JSON" GUEST_ESCAPE_PROBES_STARTED)"
GUEST_NETWORK_PROBE_STARTED="$(get_json "$OUTPUT_JSON" GUEST_NETWORK_PROBE_STARTED)"
STAGE0D_GUEST_DONE="$(get_json "$OUTPUT_JSON" STAGE0D_GUEST_DONE)"
